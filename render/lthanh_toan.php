...
if ($hd_dh != "" && $com_id != "" && $loai_phieu != "" && $id_phieu == "") {
    if ($loai_phieu == 1) {
        ...
        if ($phan_loai == 2) {
            ...
            if (mysql_num_rows($check_tt->result) > 0) {
                $tong_da_tt = mysql_fetch_assoc((new db_query("SELECT SUM(`so_tien`) AS sotien FROM `phieu_thanh_toan` WHERE `loai_phieu_tt` = 2
                                                        AND `loai_thanh_toan` = 2 AND `id_hd_dh` = $hd_dh AND `id_cong_ty` = $com_id "))->result)['sotien'];
                $gia_tri_tt = $tong_tatca - $tong_da_tt; ?>
                <div class="ctn_table">
                    <table class="table">
                        <thead>
...
} else if ($hd_dh != "" && $com_id != "" && $loai_phieu != "" && $id_phieu != "") {
    if ($loai_phieu == 1) {
        $phan_loai = mysql_fetch_assoc((new db_query("SELECT `phan_loai` FROM `hop_dong` WHERE `id_cong_ty` = $com_id AND `id` = $hd_dh "))->result)['phan_loai'];
        if ($phan_loai == 2) {
