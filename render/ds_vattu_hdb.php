...
if ($id_dh != "" && $com_id != "" && $id_hd != "" && $id_kh != "") {
    $check_tt = new db_query("SELECT `id` FROM `don_hang` WHERE `id` = $id_dh AND `id_cong_ty` = $com_id AND `id_hop_dong` = $id_hd AND `phan_loai` = 2 ");
    if (mysql_num_rows($check_tt->result) > 0) {
        ...
        $check_tt = new db_query("SELECT `id_hop_dong` FROM `don_hang` WHERE `id_cong_ty` = $com_id AND `phan_loai` = 2 AND `id_hop_dong` = $id_hd ");
        if (mysql_num_rows($check_tt->result) > 0) {
            while ($row2 = mysql_fetch_assoc($ds_vt->result)) {
...
} else if ($id_dh == "" && $com_id != "" && $id_hd != "" && $id_kh != "") {
    $ds_vt = new db_query("SELECT `id_vat_tu`, `id_hd_mua_ban`, `so_luong`, `don_gia`, `thue_vat`
                            FROM `vat_tu_hd_dh` WHERE `id_hd_mua_ban` = $id_hd ");
    $check_tt = new db_query("SELECT `id_hop_dong` FROM `don_hang` WHERE `id_cong_ty` = $com_id AND `phan_loai` = 2 AND `id_hop_dong` = $id_hd ");
    if (mysql_num_rows($check_tt->result) > 0) {
