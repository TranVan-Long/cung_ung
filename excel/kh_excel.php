...
$id_kh = getValue('id_kh', 'int', 'GET', 0);
$query = new db_query("SELECT `id`, `ten_nha_cc_kh`, `ma_so_thue`, `ten_giao_dich`, `dia_chi_dkkd`, `so_dkkd`, `dia_chi_lh`, `fax`, `so_dien_thoai`,
                    `website`, `email` FROM `nha_cc_kh` WHERE `id_cong_ty` = $com_id AND `id` = $id_kh AND `phan_loai` = 2 ");
$tai_khoan = new db_query("SELECT `ten_ngan_hang`, `ten_chi_nhanh`, `so_tk`, `chu_tk` FROM `tai_khoan` WHERE `id_nha_cc_kh` = $id_kh ");
...
echo '<tr><th colspan="2" style="font-size:18px;height:60px;vertical-align: middle;">Thông tin khách hàng: ' . $item['ten_nha_cc_kh'] . '</th></tr>';

?>
<tr style="height:40px">
    <td style="vertical-align: middle;font-size: 14px;text-align: center;width: 200px">Mã khách hàng:</td>
    <td style="vertical-align: middle;font-size: 14px;text-align: center;width: 300px;">KH - <?= $item['id'] ?></td>
</tr>
<tr style="height:40px">
...
<? while ($row = mysql_fetch_assoc($tai_khoan->result)) { ?>
    <tr style="height:40px">
        <td style="vertical-align: middle;font-size: 14px;text-align: center;width: 200px;border-top: 1px solid;">Tên ngân hàng:</td>
