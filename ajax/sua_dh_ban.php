...
if ($com_id != "" && $so_dh != "" && $id_hd != "") {
    ...
    if (mysql_num_rows($check_tt->result) > 0) {
        ...
        $upda_dhb = new db_query("UPDATE `don_hang` SET `id_nha_cc_kh`='$id_kh',`id_nguoi_lh`='$id_nguoi_lh',`id_hop_dong`='$id_hd',`id_du_an_ctrinh`='$id_ctrinh',`ngay_ky`='$ngay_ky',
        ...
        $upda_dhb = new db_query("UPDATE `don_hang` SET `id_nha_cc_kh`='$id_kh', `id_nguoi_lh`='$id_nguoi_lh', `id_hop_dong`='$id_hd', `id_du_an_ctrinh`='$id_ctrinh',
                            `ngay_ky`='$ngay_ky', `thoi_han`='$thoi_han', `don_vi_nhan_hang`='$donv_nh', `phong_ban`='$phong_ban', `nguoi_nhan_hang`='$nguoi_nh',
                            `dien_thoai_nn`='$dient_nnhan', `giu_lai_bao_hanh`='$baoh_hd', `gia_tri_tuong_duong`='$gia_tri', `ghi_chu`='$ghi_chu',
                            `gia_tri_don_hang`='$giatr_vat', `thue_vat`='$thue_vat', `gia_tri_svat`='$gias_vat', `bao_gom_vat`='$bg_gia_vat',
                            `chiet_khau`='$tien_ckhau', `chi_phi_vchuyen`='$chi_phi_vc', `ghi_chu_vchuyen`='$ghic_vc'
                            WHERE `id` = $so_dh AND `id_cong_ty` = $com_id AND `phan_loai` = 2 ");
